...
def apply_block_shift(frame, max_shift_range=(10, WIDTH // 3), block_size_range=(5, HEIGHT // 2), num_blocks_range=(20, 100)):
    ...
    for _ in range(num_blocks):
        ...
        y = random.randint(0, height - bh -1 if height - bh > 0 else 0) 
        x = random.randint(0, width - bw -1 if width - bw > 0 else 0)  
        ...
        try:
            if bh > 0 and bw > 0: # Ensure block has dimensions before slicing
...
def apply_feedback(frame, prev_frame, alpha_range=(0.02, 0.6)):
    ...
    if random.random() < 0.1:
        ...
        modified_prev_int[:,:,0] = np.clip(modified_prev_int[:,:,0] + shift_b, 0, 255)
        modified_prev_int[:,:,1] = np.clip(modified_prev_int[:,:,1] + shift_g, 0, 255)
        modified_prev_int[:,:,2] = np.clip(modified_prev_int[:,:,2] + shift_r, 0, 255)
        modified_prev = modified_prev_int.astype(np.uint8)
...
def apply_pixelation(frame, block_size_range=(4, 96)):
    ...
    if block_size == 0: block_size = 1 # Prevent division by zero
...
def apply_scanlines(frame, intensity_range=(0.4, 1.0), thickness_range=(1, 4), color_variation=40):
    ...
    if thickness <= 0: thickness = 1 # Ensure thickness is positive

    scanline_layer = np.zeros_like(frame)
    height, width = frame.shape[:2]
    base_color_val = random.randint(0, 70)

    # Iterate with a positive step size
    step = thickness * 2
    if step <= 0: step = 2 # Ensure step is positive for the range

    for y_pos in range(0, height, step): 
        line_color_val_individual = base_color_val + random.randint(-color_variation, color_variation)
        # *** FIX APPLIED HERE for cv2.line color argument ***
        # Clip the individual color value to ensure it's within 0-255
        clipped_scalar_val = int(np.clip(line_color_val_individual, 0, 255))
        # Create a tuple of standard Python integers for the color
        line_color = (clipped_scalar_val, clipped_scalar_val, clipped_scalar_val)
...
def apply_datamosh_sim(frame, prev_frame, hold_prob=0.3, smear_prob=0.5, block_size_range=(32, 128)):
    ...
    bs_max = max(bs_min + 1, block_size_range[1])
    block_size = random.randint(bs_min, bs_max)
    if block_size == 0: block_size = 16 # Ensure block_size is not zero

    if random.random() < hold_prob:
        # Ensure divisor is not zero for num_hold_blocks
        num_hold_blocks = random.randint(1, max(1, (width // block_size) * (height // block_size) // 3 if block_size > 0 else 1))
    ...
    if random.random() < smear_prob:
        num_smear_blocks = random.randint(1, max(1, (width // block_size) * (height // block_size) // 4 if block_size > 0 else 1))
        for _ in range(num_smear_blocks):
            ...
            # Ensure source block is valid before slicing prev_frame
            if by+bh > prev_frame.shape[0] or bx+bw > prev_frame.shape[1]: continue
            ...
            if target_bh > 0 and target_bw > 0 and smear_block.shape[0] >= target_bh and smear_block.shape[1] >= target_bw:
...
def apply_ascii_sim(frame, block_size_range=(8, 16), char_set=" .:-=+*#%@", invert=False):
    ...
    bs_max = max(bs_min + 1, block_size_range[1])
    block_size = random.randint(bs_min, bs_max)
    if block_size == 0: block_size = 8 # Ensure block_size is not zero
    output = frame.copy()
    ...
    for y in range(0, height - block_size + 1, block_size):
        for x in range(0, width - block_size + 1, block_size):
            if y+block_size > height or x+block_size > width: continue # Boundary check
            block_gray = gray[y:y+block_size, x:x+block_size] 
            if block_gray.size == 0: continue # Skip if block is empty
            avg_brightness = np.mean(block_gray)
            char_index = int(np.clip((avg_brightness / 255.0) * num_chars, 0, num_chars - 1 if num_chars > 0 else 0))
            char_to_draw = chars[char_index] if num_chars > 0 else " "
...
def apply_bitcrush(data, bit_depth_range=(4, 12)):
    ...
    if len(data) == 0: return data # Handle empty data
    steps = 2**(bits - 1)
...
def apply_spectral_glitch(data, intensity=0.5):
    ...
    population_size = n_freqs // 2 -1 # Avoid DC and Nyquist
    ...
    for idx in indices_to_glitch:
        if random.random() < 0.6: 
            magnitude[idx] = 0
            if n_freqs - idx < n_freqs and n_freqs - idx >=0 : magnitude[n_freqs - idx] = 0 # Symmetric for real signal
        else: 
            if idx + 1 < n_freqs // 2: # Ensure idx+1 is valid
                phase[idx], phase[idx+1] = phase[idx+1], phase[idx]
                # Mirror phase changes for real signal (conjugate symmetry)
                if n_freqs - idx < n_freqs and n_freqs - idx >=0 : phase[n_freqs - idx] = -phase[idx] 
                if n_freqs - (idx+1) < n_freqs and n_freqs - (idx+1) >=0: phase[n_freqs - (idx+1)] = -phase[idx+1]
...
def apply_convolution_reverb(data, impulse_response):
    ...
    if n_ir >= n_data : 
        ...
        if len(impulse_response_eff) == 0: return data 
...
for k_ir in IMPULSE_RESPONSES: 
    if len(IMPULSE_RESPONSES[k_ir]) > 0: 
        max_abs = np.max(np.abs(IMPULSE_RESPONSES[k_ir]))
        if max_abs > 1e-6:
            IMPULSE_RESPONSES[k_ir] /= max_abs
    else: 
        IMPULSE_RESPONSES[k_ir] = np.array([0.0]) 


# --- Tone and Noise Generation ---
intensity_function_ref_for_tone = None # Global reference for generate_tone

def generate_tone(freq, duration_samples, vol, fm_chance=0.5, harmonic_chance=0.4): 
    global intensity_function_ref_for_tone
    if duration_samples <= 0: return np.array([], dtype=np.float32) 
    t_vals = np.linspace(0, duration_samples / SAMPLE_RATE, duration_samples, endpoint=False) 
    wave = np.sin(2 * np.pi * freq * t_vals) 

    if random.random() < fm_chance and intensity_function_ref_for_tone is not None:
        # Use a representative time for intensity, e.g., midpoint of the tone's duration
        tone_mid_time = (duration_samples / SAMPLE_RATE) / 2.0
        current_intensity = intensity_function_ref_for_tone(tone_mid_time)
        mod_freq = random.uniform(1, 20) * (1 + current_intensity * 2) 
        mod_depth = random.uniform(0.1, 1.0) * freq * 0.5 
    ...
    if random.random() < harmonic_chance:
        ...
        for i_harmonic in range(2, num_harmonics + 2): 
            wave += (random.random() * 0.5) * np.sin(2 * np.pi * freq * i_harmonic * t_vals) / i_harmonic
    
    attack_len = min(duration_samples // 20 if duration_samples > 0 else 0, SAMPLE_RATE // 100) 
    decay_len = min(duration_samples // 5 if duration_samples > 0 else 0, SAMPLE_RATE // 20)   
    
    attack_len = max(0, attack_len)
    decay_len = max(0, decay_len)

    if attack_len > 0 and attack_len <= duration_samples : wave[:attack_len] *= np.linspace(0, 1, attack_len)
    
    if duration_samples > attack_len and decay_len > 0:
        decay_actual_len = min(decay_len, duration_samples - attack_len)
        if decay_actual_len > 0:
             wave[attack_len : attack_len+decay_actual_len] *= np.linspace(1, 0, decay_actual_len)
             if duration_samples > attack_len + decay_actual_len: 
                wave[attack_len+decay_actual_len:] = 0

    if len(wave) > 0:
        max_abs_wave = np.max(np.abs(wave))
        wave_normalized = wave / max_abs_wave if max_abs_wave > 1e-6 else wave
    else:
...
def generate_pink_noise(N, scale=1.0): 
    if N <= 0:
        return np.array([], dtype=np.float32) # Return float32
...
def generate_noise(noise_choice, event_duration_samples, event_vol, features):
    ...
    if noise_choice == "white":
        ...
        for _ in range(num_clicks):
            if event_duration_samples <= 1: # Need at least one sample to place a click
                if event_duration_samples == 1: noise[0] = (random.random() * 2 - 1) * event_vol * 1.5
                continue
        ...
        if event_duration_samples > glitch_len : 
            ...
            for i_glitch in range(start_pos, min(start_pos + glitch_len, event_duration_samples)): 
        ...
        t_screech = np.linspace(0, event_duration_samples / SAMPLE_RATE, event_duration_samples, endpoint=False) 
        ...
        noise = np.zeros(event_duration_samples)
        pulse_len = max(1, SAMPLE_RATE // random.randint(100, 500))
        ...
        t_feedback = np.linspace(0, event_duration_samples / SAMPLE_RATE, event_duration_samples, endpoint=False) 
    ...
    if len(noise) != event_duration_samples and event_duration_samples > 0 : 
        ...
        else: noise = np.pad(noise, (0, event_duration_samples - len(noise)), 'constant', constant_values=0.0)
    elif event_duration_samples == 0 and len(noise) != 0: # If duration is 0, noise should be empty
...
def generate_audio_enhanced(duration_samples, fps, theme_data, intensity_func_main, global_params=None): 
    global intensity_function_ref_for_tone # Make the global ref available
    intensity_function_ref_for_tone = intensity_func_main # Set it for generate_tone
    ...
    for i_event in range(num_audio_events): 
        current_time_audio = (i_event / num_audio_events if num_audio_events > 0 else 0) * (duration_samples / SAMPLE_RATE)
        intensity_factor = intensity_func_main(current_time_audio) 
        intensity_factor = np.clip(intensity_factor, 0.01, 5.0) 

        event_start_sample = random.randint(0, max(0, duration_samples - (SAMPLE_RATE // 10) -1 if duration_samples > (SAMPLE_RATE // 10) else 0 ))
        ...
        for feature_name in theme_data.get("audio_features", []):
            ...
            if feature_name == "stutter" and random.random() < 0.2 * intensity_factor:
                stutter_len = min(len(segment_mono)//2 if len(segment_mono)>1 else 0, max(1, SAMPLE_RATE // random.randint(50,200)))
                if stutter_len > 0 and len(segment_mono) > stutter_len: # Ensure stutter_len valid and segment is long enough
                    ...
                    remaining_part = segment_mono[stutter_len:]
                    segment_mono = np.concatenate((stuttered_part, remaining_part))
                    if len(segment_mono) > original_len: 
                        segment_mono = segment_mono[:original_len]
                    elif len(segment_mono) < original_len and original_len > 0: # Pad if it became shorter
                        segment_mono = np.pad(segment_mono, (0, original_len - len(segment_mono)), 'constant', constant_values=0.0)
            ...
            if "reverb_ir" in features and random.random() < 0.5 and len(features["reverb_ir"]) > 0 and np.any(features["reverb_ir"]): # Ensure IR is not all zeros
        ...
        if end_sample > duration_samples:
            ...
            segment_mono = segment_mono[:actual_len] # Ensure segment_mono is sliced if too long
            end_sample = duration_samples # Update end_sample based on actual_len
        
        if actual_len > 0: # Ensure segment has length before adding
            audio_data_stereo[event_start_sample:end_sample, 0] += segment_mono 
            audio_data_stereo[event_start_sample:end_sample, 1] += segment_mono 
            # Apply gains after adding to avoid issues if segment_mono is shorter than slice
            audio_data_stereo[event_start_sample:end_sample, 0] *= left_gain 
            audio_data_stereo[event_start_sample:end_sample, 1] *= right_gain


    max_abs_val = np.max(np.abs(audio_data_stereo)) if duration_samples > 0 and audio_data_stereo.size > 0 else 0
    if max_abs_val > 1.0:
...
def generate_melody_samples(duration_samples, theme_data, current_intensity_val, sample_rate): 
    ...
    min_notes = 1
    # Ensure max_notes is at least min_notes, and calculation doesn't underflow for small duration_samples
    max_notes_calc = duration_samples // (sample_rate // 8 if sample_rate > 0 else duration_samples + 1) # Avoid div by zero
    max_notes = max(min_notes, max_notes_calc if max_notes_calc > 0 else min_notes)
    num_notes = random.randint(min_notes, max_notes)
    
    if num_notes == 0 : return np.zeros(duration_samples, dtype=np.float32)
    
    note_duration_samples = duration_samples // num_notes if num_notes > 0 else duration_samples
    if note_duration_samples == 0 and duration_samples > 0 : 
        note_duration_samples = duration_samples # Ensure at least one note if duration exists
        num_notes = 1
    elif note_duration_samples == 0 and duration_samples == 0:
        return np.array([], dtype=np.float32)


    melody = np.array([], dtype=np.float32)
    for _ in range(num_notes):
        if note_duration_samples <=0 : continue # Skip if calculated note duration is zero
        ...
        current_freq = np.clip(current_freq, LOW_FREQ_CUTOFF, HIGH_FREQ_CUTOFF / 2 if HIGH_FREQ_CUTOFF > 0 else LOW_FREQ_CUTOFF +1) 
        ...
        attack_len = min(note_duration_samples // 10 if note_duration_samples >0 else 0, sample_rate // 100 if sample_rate >0 else 0) 
        decay_len = min(note_duration_samples // 5 if note_duration_samples >0 else 0, sample_rate // 50 if sample_rate >0 else 0)
        sustain_level = 0.7
        release_len = min(note_duration_samples // 10 if note_duration_samples >0 else 0, sample_rate // 100 if sample_rate >0 else 0)
        
        attack_len = max(0, attack_len)
        decay_len = max(0, decay_len)
        release_len = max(0, release_len)

        if attack_len > 0 and attack_len <= note_duration_samples: 
            note_wave[:attack_len] *= np.linspace(0, 1, attack_len)
        
        sustain_start_idx = attack_len + decay_len # Renamed sustain_start
        release_start_idx = note_duration_samples - release_len # Renamed release_start

        if decay_len > 0 and attack_len < sustain_start_idx and sustain_start_idx <= note_duration_samples :
             # Ensure length of linspace matches slice
             decay_slice_len = sustain_start_idx - attack_len
             if decay_slice_len > 0:
                note_wave[attack_len : sustain_start_idx] *= np.linspace(1, sustain_level, decay_slice_len)
        
        if sustain_start_idx < release_start_idx and release_start_idx <= note_duration_samples: 
            note_wave[sustain_start_idx : release_start_idx] *= sustain_level
        
        if release_len > 0 and release_start_idx < note_duration_samples : 
            # Ensure length of linspace matches slice
            release_slice_len = note_duration_samples - release_start_idx
            if release_slice_len > 0:
                note_wave[release_start_idx:] *= np.linspace(sustain_level if release_start_idx >= sustain_start_idx else 1.0, 0, release_slice_len) # Adjust start of linspace if sustain wasn't hit

        melody = np.concatenate((melody, note_wave))

    distortion_factor = theme_data.get("melody_distortion", 0) * current_intensity_val # Renamed
    if distortion_factor > 0.1 and len(melody) > 0:
        melody = np.clip(melody * (1 + distortion_factor), -0.98, 0.98) 

    if len(melody) > duration_samples:
        melody = melody[:duration_samples]
    elif len(melody) < duration_samples and duration_samples > 0 : 
        melody = np.pad(melody, (0, duration_samples - len(melody)), 'constant', constant_values=0.0)
...
def generate_video():
    ...
    if actual_fps == 0: actual_fps = 15 # Prevent FPS from being zero
    ...
    main_intensity_func = generate_intensity_profile(actual_duration_samples, SAMPLE_RATE)
    intensity_function_ref_for_tone = main_intensity_func # Set the global reference for generate_tone
    ...
    for _ in range(num_word_elements):
        word = random.choice(current_theme_data["words"]) if current_theme_data.get("words") else "GLITCH"
        if random.random() < current_theme_data.get("zalgo_words_chance", 0):
            word = zalgo_text(word)
        
        symbols_list = current_theme_data.get("symbols", ["!", "?"]) # Default symbols if not present
        symbol_prefix = random.choice(symbols_list) if symbols_list and random.random() < 0.7 else ""
        symbol_suffix = random.choice(symbols_list) if symbols_list and random.random() < 0.7 else ""
        ...
        current_palette_choices = current_theme_data.get("colors", [[(0,0,0), (255,255,255)]]) # Default palette
        if not current_palette_choices: 
            current_palette_choices = [[(0,0,0), (255,255,255)]] # Ensure it's never empty
        
        chosen_palette = random.choice(current_palette_choices)
        if not chosen_palette: # If a palette itself is an empty list
            text_color_val = (255,255,255)
        elif len(chosen_palette) > 1:
             color_idx = random.randint(1, len(chosen_palette) -1) 
             text_color_val = chosen_palette[color_idx] 
        elif len(chosen_palette) == 1: 
        ...
        # Ensure pos_x and pos_y ranges are valid
        pos_x_max = max(WIDTH // 10 + 1, WIDTH - WIDTH // 5)
        pos_y_max = max(HEIGHT // 10 + 1, HEIGHT - HEIGHT // 5)
        pos_x = random.randint(WIDTH // 10, pos_x_max)
        pos_y = random.randint(HEIGHT // 10, pos_y_max)
        text_elements.append({"text": display_text, "scale": font_scale, "color": text_color_val, "thick": thickness, "pos": (pos_x, pos_y)})


    for frame_num in range(total_frames):
        current_time_video = frame_num / actual_fps if actual_fps > 0 else 0
        current_intensity_val = main_intensity_func(current_time_video) 
        current_intensity_val = np.clip(current_intensity_val, 0.01, 5.0) 

        palettes_available = current_theme_data.get("colors", [[(0,0,0), (255,255,255)]])
        if not palettes_available: palettes_available = [[(0,0,0), (255,255,255)]]
        palette = random.choice(palettes_available) 
        
        frame = base_frame.copy()
        frame[:,:] = palette[0] if palette and len(palette)>0 and palette[0] else (0,0,0)

        if random.random() < 0.2 * current_intensity_val: 
            num_shapes = random.randint(1, max(1, int(5 * current_intensity_val)))
            for _ in range(num_shapes):
                shape_type = random.choice(["rect", "line", "circle"])
                fg_colors = palette[1:] if palette and len(palette) > 1 else [(255,255,255)] 
                if not fg_colors: fg_colors = [(255,255,255)] # Ensure fg_colors is not empty
                color = random.choice(fg_colors)
                x1, y1 = random.randint(0, WIDTH), random.randint(0, HEIGHT)
                x2, y2 = random.randint(0, WIDTH), random.randint(0, HEIGHT)
                thick = random.randint(1, max(1, int(10 * current_intensity_val)))
                if shape_type == "rect":
        ...
        num_effects_to_apply = random.randint(1, max(1, 2 + int(current_intensity_val)))
        ...
        num_visible_texts = random.randint(0, max(0, 2 + int(current_intensity_val*2)))
        sample_k = min(num_visible_texts, len(text_elements))
        visible_texts = random.sample(text_elements, sample_k) if len(text_elements) > 0 and sample_k > 0 else []
        ...
        if frame_num % (actual_fps * 5 if actual_fps > 0 else 5) == 0 : 
    ...
    if audio_temp_file and os.path.exists(OUTPUT_FILE) and os.path.exists(audio_temp_file): 
        ...
        try:
            ...
            except OSError as e: print(f"Error removing temporary video file {OUTPUT_FILE}: {e}")
            try: os.remove(audio_temp_file)
            except OSError as e: print(f"Error removing temporary audio file {audio_temp_file}: {e}")
            ...
            if audio_temp_file: print(f"Audio-only file is at: {audio_temp_file}")
    else:
